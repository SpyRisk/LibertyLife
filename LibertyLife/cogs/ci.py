...
#### SQL #####
conn = sqlite3.connect('Eve-Dj.db') #Connexion SQL
...
class Identity:
    ...
    async def _ci(self, ctx):
        ...
        if ctx.invoked_subcommand is None:
            ...
            em = discord.Embed(title='Commandes de carte d\'identité de Eve-Dj', description=text, colour=0x89C4F9)
    ...
    async def ci_test(self, ctx, args : discord.Member):
        ...
        if not result:
            ...
            try:
                ...
                embed=discord.Embed(title="Carte d'identité | Liberty Life")
                ...
                embed.add_field(name="Prénom :", value=isexist(result[6]), inline=True)
                embed.add_field(name="Age : ", value=isexist(result[7]), inline=True)
                embed.add_field(name="Date de naissance : ", value=userbirth[0], inline=True)
                embed.add_field(name="Nationalité : ", value=isexist(result[8]), inline=True)
    ...
    async def ci_register(self, ctx):
        ...
        if existansw != None:
            ...
            conn.commit()
            await ctx.send(":clap: Bievenue à toi {} dans {} ! Fait ``.ci`` pour plus d'informations !".format(ctx.message.author.name, str(ctx.message.guild.name)))
    ...
    @_ci.command(pass_context=True, name="name")
    async def ci_setconfig(self, ctx, args_):
        try:
            args = ctx.message.content.split("Nom")
            args = args[1]
            cursor.execute("""SELECT id, userid FROM users WHERE userid=?""", (ctx.message.author.id,))
            existansw = cursor.fetchone()
    ...
    @_ci.command(pass_context=True, name="Age")
    async def ci_setos(self, ctx, args_):
        try:
            args = ctx.message.content.split("Age ")
            args = args[1]
            cursor.execute("""SELECT id, userid FROM users WHERE userid=?""", (ctx.message.author.id,))
            existansw = cursor.fetchone()
    ...
    @_ci.command(pass_context=True, name="nat")
